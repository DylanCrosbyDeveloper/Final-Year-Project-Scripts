...
public class LobbyManager : MonoBehaviour
{
    ...
    public async void LobbyStart()
    {
        ...
        if (createLobbyGameModeDropdown.value == 0)
        {
            networkManager.SceneManager.LoadScene("Deathmatch", LoadSceneMode.Single);
        }

        if (createLobbyGameModeDropdown.value == 1)
        {
            networkManager.SceneManager.LoadScene("TeamDeathmatch", LoadSceneMode.Single);
        }

        if (createLobbyGameModeDropdown.value == 2)
        {
            networkManager.SceneManager.LoadScene("CaptureTheFlag", LoadSceneMode.Single);
        }
        else return;
            //networkManager.SceneManager.LoadScene("TestScene", LoadSceneMode.Additive);
        lobbyListParent.SetActive(false);
        joinedLobbyParent.SetActive(false);
    }
